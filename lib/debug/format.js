...
function format_result(iters){
  ...
  return typeof returned === 'function' ? 'function' : format_array(returned.map(putils.format_argument)) ;
}

function invo_sum(iters){
  var f_args_hist = iters.from.f_args;
  return f_args_hist.map(putils.format_invocation).join('');
}
...
function format_stock(stock, head, tail, args_count, instance_mark){
  var stock_s = putils.format_arguments(stock);
