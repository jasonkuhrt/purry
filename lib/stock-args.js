...
function stock_args(f, _params, _psize, _phead, _ptail){
  ...
  function intercept_arguments(){
    ...
    if (!arguments_size) {
      ...
      // debug.bar();
      // f.purry_history = f.purry_history.concat([{params:_params, args: Array.prototype.slice.call(arguments, 0)}]);
      return _psize ? intercept_arguments : f.apply(null, _params)  ;

    // Due to delaying its possible that this
    // function is fully stocked. If so it
    // does not accept any more arguments, so
    // throw error!
    } else if (_psize === 0) {
    ...
    // f.purry_history = f.purry_history.concat([{params:params, args: Array.prototype.slice.call(arguments, 0)}]);
